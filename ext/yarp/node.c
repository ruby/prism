...
yp_node_new(yp_parser_t *parser, yp_node_t *node, rb_encoding *encoding) {
  switch (node->type) {
    case YP_NODE_ARRAY_PATTERN_NODE: {
      ...
      // constant
      argv[0] = node->as.array_pattern_node.constant == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.array_pattern_node.constant, encoding);

      // requireds
      argv[1] = rb_ary_new();
      for (size_t index = 0; index < node->as.array_pattern_node.requireds.size; index++) {
        rb_ary_push(argv[1], yp_node_new(parser, node->as.array_pattern_node.requireds.nodes[index], encoding));
      }

      // rest
      argv[2] = node->as.array_pattern_node.rest == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.array_pattern_node.rest, encoding);

      // posts
      argv[3] = rb_ary_new();
      for (size_t index = 0; index < node->as.array_pattern_node.posts.size; index++) {
        rb_ary_push(argv[3], yp_node_new(parser, node->as.array_pattern_node.posts.nodes[index], encoding));
      }

      // opening_loc
      argv[4] = node->as.array_pattern_node.opening_loc.start == NULL ? Qnil : location_new(parser, node->as.array_pattern_node.opening_loc.start, node->as.array_pattern_node.opening_loc.end);

      // closing_loc
      argv[5] = node->as.array_pattern_node.closing_loc.start == NULL ? Qnil : location_new(parser, node->as.array_pattern_node.closing_loc.start, node->as.array_pattern_node.closing_loc.end);
    ...
    case YP_NODE_AS_PATTERN_NODE: {
      ...
      // value
      argv[0] = yp_node_new(parser, (yp_node_t *)node->as.as_pattern_node.value, encoding);

      // target
      argv[1] = yp_node_new(parser, (yp_node_t *)node->as.as_pattern_node.target, encoding);

      // operator_loc
      argv[2] = location_new(parser, node->as.as_pattern_node.operator_loc.start, node->as.as_pattern_node.operator_loc.end);

      // location
      argv[3] = location_new(parser, node->location.start, node->location.end);

      return rb_class_new_instance(4, argv, rb_const_get_at(rb_cYARP, rb_intern("AsPatternNode")));
    }
    case YP_NODE_ASSOC_NODE: {
      VALUE argv[4];

      // key
      argv[0] = yp_node_new(parser, (yp_node_t *)node->as.assoc_node.key, encoding);

      // value
      argv[1] = node->as.assoc_node.value == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.assoc_node.value, encoding);

      // operator
      argv[2] = node->as.assoc_node.operator.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.assoc_node.operator, encoding);
    ...
    case YP_NODE_ASSOC_SPLAT_NODE: {
      ...
      // value
      argv[0] = node->as.assoc_splat_node.value == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.assoc_splat_node.value, encoding);

      // operator_loc
      argv[1] = location_new(parser, node->as.assoc_splat_node.operator_loc.start, node->as.assoc_splat_node.operator_loc.end);
    ...
    case YP_NODE_CALL_NODE: {
      ...
      // receiver
      argv[0] = node->as.call_node.receiver == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.call_node.receiver, encoding);

      // call_operator
      argv[1] = node->as.call_node.call_operator.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.call_node.call_operator, encoding);

      // message
      argv[2] = node->as.call_node.message.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.call_node.message, encoding);

      // opening
      argv[3] = node->as.call_node.opening.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.call_node.opening, encoding);

      // arguments
      argv[4] = node->as.call_node.arguments == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.call_node.arguments, encoding);

      // closing
      argv[5] = node->as.call_node.closing.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.call_node.closing, encoding);

      // block
      argv[6] = node->as.call_node.block == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.call_node.block, encoding);

      // name
      argv[7] = yp_string_new(&node->as.call_node.name, encoding);
    ...
    case YP_NODE_CLASS_VARIABLE_WRITE_NODE: {
      ...
      // name_loc
      argv[0] = location_new(parser, node->as.class_variable_write_node.name_loc.start, node->as.class_variable_write_node.name_loc.end);

      // value
      argv[1] = node->as.class_variable_write_node.value == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.class_variable_write_node.value, encoding);

      // operator_loc
      argv[2] = node->as.class_variable_write_node.operator_loc.start == NULL ? Qnil : location_new(parser, node->as.class_variable_write_node.operator_loc.start, node->as.class_variable_write_node.operator_loc.end);
    ...
    case YP_NODE_CONSTANT_PATH_WRITE_NODE: {
      ...
      // target
      argv[0] = yp_node_new(parser, (yp_node_t *)node->as.constant_path_write_node.target, encoding);

      // operator
      argv[1] = node->as.constant_path_write_node.operator.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.constant_path_write_node.operator, encoding);

      // value
      argv[2] = node->as.constant_path_write_node.value == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.constant_path_write_node.value, encoding);
    ...
    case YP_NODE_FIND_PATTERN_NODE: {
      ...
      // constant
      argv[0] = node->as.find_pattern_node.constant == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.find_pattern_node.constant, encoding);

      // left
      argv[1] = yp_node_new(parser, (yp_node_t *)node->as.find_pattern_node.left, encoding);

      // requireds
      argv[2] = rb_ary_new();
      for (size_t index = 0; index < node->as.find_pattern_node.requireds.size; index++) {
        rb_ary_push(argv[2], yp_node_new(parser, node->as.find_pattern_node.requireds.nodes[index], encoding));
      }

      // right
      argv[3] = yp_node_new(parser, (yp_node_t *)node->as.find_pattern_node.right, encoding);

      // opening_loc
      argv[4] = node->as.find_pattern_node.opening_loc.start == NULL ? Qnil : location_new(parser, node->as.find_pattern_node.opening_loc.start, node->as.find_pattern_node.opening_loc.end);

      // closing_loc
      argv[5] = node->as.find_pattern_node.closing_loc.start == NULL ? Qnil : location_new(parser, node->as.find_pattern_node.closing_loc.start, node->as.find_pattern_node.closing_loc.end);
    ...
    case YP_NODE_GLOBAL_VARIABLE_READ_NODE: {
      ...
      // name
      argv[0] = yp_token_new(parser, &node->as.global_variable_read_node.name, encoding);
    ...
    case YP_NODE_GLOBAL_VARIABLE_WRITE_NODE: {
      ...
      // name
      argv[0] = yp_token_new(parser, &node->as.global_variable_write_node.name, encoding);

      // operator
      argv[1] = node->as.global_variable_write_node.operator.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.global_variable_write_node.operator, encoding);

      // value
      argv[2] = node->as.global_variable_write_node.value == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.global_variable_write_node.value, encoding);
    ...
    case YP_NODE_HASH_NODE: {
      ...
      // opening
      argv[0] = node->as.hash_node.opening.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.hash_node.opening, encoding);

      // elements
      argv[1] = rb_ary_new();
      for (size_t index = 0; index < node->as.hash_node.elements.size; index++) {
        rb_ary_push(argv[1], yp_node_new(parser, node->as.hash_node.elements.nodes[index], encoding));
      }

      // closing
      argv[2] = node->as.hash_node.closing.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.hash_node.closing, encoding);
    ...
    case YP_NODE_HASH_PATTERN_NODE: {
      ...
      // constant
      argv[0] = node->as.hash_pattern_node.constant == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.hash_pattern_node.constant, encoding);

      // assocs
      argv[1] = rb_ary_new();
      for (size_t index = 0; index < node->as.hash_pattern_node.assocs.size; index++) {
        rb_ary_push(argv[1], yp_node_new(parser, node->as.hash_pattern_node.assocs.nodes[index], encoding));
      }

      // kwrest
      argv[2] = node->as.hash_pattern_node.kwrest == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.hash_pattern_node.kwrest, encoding);

      // opening_loc
      argv[3] = node->as.hash_pattern_node.opening_loc.start == NULL ? Qnil : location_new(parser, node->as.hash_pattern_node.opening_loc.start, node->as.hash_pattern_node.opening_loc.end);

      // closing_loc
      argv[4] = node->as.hash_pattern_node.closing_loc.start == NULL ? Qnil : location_new(parser, node->as.hash_pattern_node.closing_loc.start, node->as.hash_pattern_node.closing_loc.end);
    ...
    case YP_NODE_HEREDOC_NODE: {
      ...
      // opening
      argv[0] = yp_token_new(parser, &node->as.heredoc_node.opening, encoding);

      // parts
      argv[1] = rb_ary_new();
      for (size_t index = 0; index < node->as.heredoc_node.parts.size; index++) {
        rb_ary_push(argv[1], yp_node_new(parser, node->as.heredoc_node.parts.nodes[index], encoding));
      }

      // closing
      argv[2] = yp_token_new(parser, &node->as.heredoc_node.closing, encoding);

      // dedent
      argv[3] = INT2FIX(node->as.heredoc_node.dedent);

      // location
      argv[4] = location_new(parser, node->location.start, node->location.end);

      return rb_class_new_instance(5, argv, rb_const_get_at(rb_cYARP, rb_intern("HeredocNode")));
    }
    case YP_NODE_IF_NODE: {
      VALUE argv[6];

      // if_keyword
      argv[0] = yp_token_new(parser, &node->as.if_node.if_keyword, encoding);

      // predicate
      argv[1] = yp_node_new(parser, (yp_node_t *)node->as.if_node.predicate, encoding);

      // statements
      argv[2] = node->as.if_node.statements == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.if_node.statements, encoding);

      // consequent
      argv[3] = node->as.if_node.consequent == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.if_node.consequent, encoding);

      // end_keyword
      argv[4] = node->as.if_node.end_keyword.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.if_node.end_keyword, encoding);
    ...
    case YP_NODE_IMAGINARY_NODE: {
      ...
      return rb_class_new_instance(1, argv, rb_const_get_at(rb_cYARP, rb_intern("ImaginaryNode")));
    }
    case YP_NODE_IN_NODE: {
      VALUE argv[5];

      // pattern
      argv[0] = yp_node_new(parser, (yp_node_t *)node->as.in_node.pattern, encoding);

      // statements
      argv[1] = node->as.in_node.statements == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.in_node.statements, encoding);

      // in_loc
      argv[2] = location_new(parser, node->as.in_node.in_loc.start, node->as.in_node.in_loc.end);

      // then_loc
      argv[3] = node->as.in_node.then_loc.start == NULL ? Qnil : location_new(parser, node->as.in_node.then_loc.start, node->as.in_node.then_loc.end);
    ...
    case YP_NODE_INSTANCE_VARIABLE_WRITE_NODE: {
      ...
      // name_loc
      argv[0] = location_new(parser, node->as.instance_variable_write_node.name_loc.start, node->as.instance_variable_write_node.name_loc.end);

      // value
      argv[1] = node->as.instance_variable_write_node.value == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.instance_variable_write_node.value, encoding);

      // operator_loc
      argv[2] = node->as.instance_variable_write_node.operator_loc.start == NULL ? Qnil : location_new(parser, node->as.instance_variable_write_node.operator_loc.start, node->as.instance_variable_write_node.operator_loc.end);
    ...
    case YP_NODE_INTERPOLATED_REGULAR_EXPRESSION_NODE: {
      ...
      // opening
      argv[0] = yp_token_new(parser, &node->as.interpolated_regular_expression_node.opening, encoding);

      // parts
      argv[1] = rb_ary_new();
      for (size_t index = 0; index < node->as.interpolated_regular_expression_node.parts.size; index++) {
        rb_ary_push(argv[1], yp_node_new(parser, node->as.interpolated_regular_expression_node.parts.nodes[index], encoding));
      }

      // closing
      argv[2] = yp_token_new(parser, &node->as.interpolated_regular_expression_node.closing, encoding);
    ...
    case YP_NODE_INTERPOLATED_STRING_NODE: {
      ...
      // opening
      argv[0] = node->as.interpolated_string_node.opening.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.interpolated_string_node.opening, encoding);

      // parts
      argv[1] = rb_ary_new();
      for (size_t index = 0; index < node->as.interpolated_string_node.parts.size; index++) {
        rb_ary_push(argv[1], yp_node_new(parser, node->as.interpolated_string_node.parts.nodes[index], encoding));
      }

      // closing
      argv[2] = node->as.interpolated_string_node.closing.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.interpolated_string_node.closing, encoding);
    ...
    case YP_NODE_INTERPOLATED_SYMBOL_NODE: {
      ...
      // opening
      argv[0] = node->as.interpolated_symbol_node.opening.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.interpolated_symbol_node.opening, encoding);

      // parts
      argv[1] = rb_ary_new();
      for (size_t index = 0; index < node->as.interpolated_symbol_node.parts.size; index++) {
        rb_ary_push(argv[1], yp_node_new(parser, node->as.interpolated_symbol_node.parts.nodes[index], encoding));
      }

      // closing
      argv[2] = node->as.interpolated_symbol_node.closing.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.interpolated_symbol_node.closing, encoding);
    ...
    case YP_NODE_INTERPOLATED_X_STRING_NODE: {
      ...
      // opening
      argv[0] = yp_token_new(parser, &node->as.interpolated_x_string_node.opening, encoding);

      // parts
      argv[1] = rb_ary_new();
      for (size_t index = 0; index < node->as.interpolated_x_string_node.parts.size; index++) {
        rb_ary_push(argv[1], yp_node_new(parser, node->as.interpolated_x_string_node.parts.nodes[index], encoding));
      }

      // closing
      argv[2] = yp_token_new(parser, &node->as.interpolated_x_string_node.closing, encoding);
    ...
    case YP_NODE_KEYWORD_PARAMETER_NODE: {
      ...
      // name
      argv[0] = yp_token_new(parser, &node->as.keyword_parameter_node.name, encoding);

      // value
      argv[1] = node->as.keyword_parameter_node.value == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.keyword_parameter_node.value, encoding);
    ...
    case YP_NODE_KEYWORD_REST_PARAMETER_NODE: {
      ...
      // operator
      argv[0] = yp_token_new(parser, &node->as.keyword_rest_parameter_node.operator, encoding);

      // name
      argv[1] = node->as.keyword_rest_parameter_node.name.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.keyword_rest_parameter_node.name, encoding);
    ...
    case YP_NODE_LAMBDA_NODE: {
      ...
      // scope
      argv[0] = yp_node_new(parser, (yp_node_t *)node->as.lambda_node.scope, encoding);

      // opening
      argv[1] = yp_token_new(parser, &node->as.lambda_node.opening, encoding);

      // lparen
      argv[2] = node->as.lambda_node.lparen.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.lambda_node.lparen, encoding);

      // parameters
      argv[3] = node->as.lambda_node.parameters == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.lambda_node.parameters, encoding);

      // rparen
      argv[4] = node->as.lambda_node.rparen.type == YP_TOKEN_NOT_PROVIDED ? Qnil : yp_token_new(parser, &node->as.lambda_node.rparen, encoding);

      // statements
      argv[5] = node->as.lambda_node.statements == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)node->as.lambda_node.statements, encoding);

      // location
      argv[6] = location_new(parser, node->location.start, node->location.end);

      return rb_class_new_instance(7, argv, rb_const_get_at(rb_cYARP, rb_intern("LambdaNode")));
    }
    ...
    case YP_NODE_BLOCK_PARAMETERS_NODE: {
      ...
      return rb_class_new_instance(3, argv, rb_const_get_at(rb_cYARP, rb_intern("BlockParametersNode")));
    }
    ...
    case YP_NODE_LOCAL_VARIABLE_READ_NODE: {
      ...
      return rb_class_new_instance(1, argv, rb_const_get_at(rb_cYARP, rb_intern("LocalVariableReadNode")));
    }
    case YP_NODE_LOCAL_VARIABLE_WRITE_NODE: {
      ...
      return rb_class_new_instance(4, argv, rb_const_get_at(rb_cYARP, rb_intern("LocalVariableWriteNode")));
    }
    ...
    case YP_NODE_PARAMETERS_NODE: {
      ...
      // rest
      argv[2] = ((yp_parameters_node_t *)node)->rest == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)((yp_parameters_node_t *)node)->rest, encoding);

      // keywords
      argv[3] = rb_ary_new();
      for (size_t index = 0; index < ((yp_parameters_node_t *)node)->keywords.size; index++) {
        rb_ary_push(argv[3], yp_node_new(parser, ((yp_parameters_node_t *)node)->keywords.nodes[index], encoding));
      }

      // keyword_rest
      argv[4] = ((yp_parameters_node_t *)node)->keyword_rest == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)((yp_parameters_node_t *)node)->keyword_rest, encoding);

      // block
      argv[5] = ((yp_parameters_node_t *)node)->block == NULL ? Qnil : yp_node_new(parser, (yp_node_t *)((yp_parameters_node_t *)node)->block, encoding);

      // location
      argv[6] = location_new(parser, node->location.start, node->location.end);

      return rb_class_new_instance(7, argv, rb_const_get_at(rb_cYARP, rb_intern("ParametersNode")));
    }
    ...
    case YP_NODE_RATIONAL_NODE: {
      ...
      return rb_class_new_instance(1, argv, rb_const_get_at(rb_cYARP, rb_intern("RationalNode")));
    }
    ...
    case YP_NODE_SOURCE_FILE_NODE: {
      ...
      return rb_class_new_instance(1, argv, rb_const_get_at(rb_cYARP, rb_intern("SourceFileNode")));
    }
