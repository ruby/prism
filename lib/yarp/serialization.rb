  ...
  module Serialization
    class << self
      ...
      def load_node(source, io)
        ...
        when 2 then YARP::CallNode.new(load_token(source, io), location)
        when 3 then YARP::CharacterLiteral.new(load_token(source, io), location)
        when 4 then YARP::ClassVariableRead.new(load_token(source, io), location)
        when 5 then YARP::ClassVariableWrite.new(load_token(source, io), load_token(source, io), load_node(source, io), location)
        when 6 then YARP::FalseNode.new(load_token(source, io), location)
        when 7 then YARP::FloatLiteral.new(load_token(source, io), location)
        when 8 then YARP::GlobalVariableRead.new(load_token(source, io), location)
        when 9 then YARP::GlobalVariableWrite.new(load_token(source, io), load_token(source, io), load_node(source, io), location)
        ...
        when 15 then YARP::LocalVariableRead.new(load_token(source, io), location)
        when 16 then YARP::LocalVariableWrite.new(load_token(source, io), load_token(source, io), load_node(source, io), location)
        when 17 then YARP::NilNode.new(load_token(source, io), location)
        when 18 then YARP::OperatorAssignment.new(load_node(source, io), load_token(source, io), load_node(source, io), location)
        when 19 then YARP::Program.new(load_node(source, io), load_node(source, io), location)
        when 20 then YARP::RationalLiteral.new(load_token(source, io), location)
        when 21 then YARP::Redo.new(load_token(source, io), location)
        when 22 then YARP::Retry.new(load_token(source, io), location)
        when 23 then YARP::Scope.new(io.read(8).unpack1("Q").times.map { load_token(source, io) }, location)
        when 24 then YARP::SelfNode.new(load_token(source, io), location)
        when 25 then YARP::Statements.new(io.read(8).unpack1("Q").times.map { load_node(source, io) }, location)
        when 26 then YARP::Ternary.new(load_node(source, io), load_token(source, io), load_node(source, io), load_token(source, io), load_node(source, io), location)
        when 27 then YARP::TrueNode.new(load_token(source, io), location)
        when 28 then YARP::UnlessNode.new(load_token(source, io), load_node(source, io), load_node(source, io), location)
        when 29 then YARP::UntilNode.new(load_token(source, io), load_node(source, io), load_node(source, io), location)
        when 30 then YARP::WhileNode.new(load_token(source, io), load_node(source, io), load_node(source, io), location)
        end
      end
    end
  end
