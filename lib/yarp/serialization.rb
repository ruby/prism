  ...
  module Serialization
    class << self
      ...
      def load_node(source, io)
        ...
        when 15 then YARP::LocalVariableWrite.new(load_token(source, io), load_token(source, io), load_node(source, io), location)
        when 16 then YARP::NilNode.new(load_token(source, io), location)
        when 17 then YARP::OperatorAssignment.new(load_node(source, io), load_token(source, io), load_node(source, io), location)
        when 18 then YARP::Program.new(load_node(source, io), load_node(source, io), location)
        ...
        when 22 then YARP::Scope.new(io.read(8).unpack1("Q").times.map { load_token(source, io) }, location)
        when 23 then YARP::SelfNode.new(load_token(source, io), location)
        when 24 then YARP::Statements.new(io.read(8).unpack1("Q").times.map { load_node(source, io) }, location)
        when 25 then YARP::Ternary.new(load_node(source, io), load_token(source, io), load_node(source, io), load_token(source, io), load_node(source, io), location)
        when 26 then YARP::TrueNode.new(load_token(source, io), location)
        when 27 then YARP::UnlessNode.new(load_token(source, io), load_node(source, io), load_node(source, io), location)
        when 28 then YARP::UntilNode.new(load_token(source, io), load_node(source, io), load_node(source, io), location)
        when 29 then YARP::WhileNode.new(load_token(source, io), load_node(source, io), load_node(source, io), location)
        end
      end
    end
  end
