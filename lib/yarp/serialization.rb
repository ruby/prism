  ...
  module Serialization
    class << self
      ...
      def load_node(source, io)
        ...
        when 9 then YARP::IfNode.new(load_token(source, io), load_node(source, io), load_node(source, io), location)
        when 10 then YARP::ImaginaryLiteral.new(load_token(source, io), location)
        when 11 then YARP::InstanceVariableRead.new(load_token(source, io), location)
        when 12 then YARP::InstanceVariableWrite.new(load_token(source, io), load_token(source, io), load_node(source, io), location)
        when 13 then YARP::IntegerLiteral.new(load_token(source, io), location)
        when 14 then YARP::NilNode.new(load_token(source, io), location)
        when 15 then YARP::OperatorAssignment.new(load_node(source, io), load_token(source, io), load_node(source, io), location)
        when 16 then YARP::Program.new(load_node(source, io), location)
        when 17 then YARP::RationalLiteral.new(load_token(source, io), location)
        when 18 then YARP::Redo.new(load_token(source, io), location)
        when 19 then YARP::Retry.new(load_token(source, io), location)
        when 20 then YARP::SelfNode.new(load_token(source, io), location)
